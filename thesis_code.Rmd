...
deaths <- deathsolder %>% rbind(deaths39) %>% filter(CO_REGION!="N/A") %>% arrange(factor(TX_GRUPO_EDAD, levels = ages2)) %>% arrange(factor(CO_REGION, levels = regions)) %>% rename(Region=CO_REGION) %>% rename(Age = TX_GRUPO_EDAD) %>%       
    mutate(Age=lapply(Age,ages_fun))
...
cfr <- cases_deaths_pop %>% mutate(st_pop = lapply(Age, st_pop_function))
...
#adding PR ypll
deaths2 <- deaths %>% filter(Age!="80 +") %>% mutate(age_start = as.numeric(str_extract(Age, "^\\d+")), age_end = as.numeric(str_extract(Age, "\\d+$")))

ypll <- deaths2 %>%
  mutate(ypll = 80 - (1 + age_end + age_start)/2) %>%
